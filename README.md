...
**Data Preparation:**
The dataset is split into a **discovery data set (80%)** and a **target data set (20%)** to ensure consistency across genetic, outcome, exposure, and confounder variables. Genetic data must be provided in **PLINK binary format** (.bed, .bim, .fam). The outcome file should contain **FID, IID, and outcome values**, with case-control coding as follows: in the discovery dataset, use PLINK's default (**1 = Control, 2 = Case**), and in the target dataset, use binary coding (**0 = Control, 1 = Case**). Exposure and confounder variables should be organized in at least **m columns** with the format: FID, IID, exposure, exposure_square, confounder1 to confounder_m, for the discovery dataset(GxEprs). The Exposure square is not necessary for the target datasets.  The Exposure variables in the discovery data set also further prepared as **FID, IID, and exposure values** and the covariate for adjustment as **FID, IID, and conf_1, constant_Values, Conf_2, ...conf_m** for conducting analysis for GWAS of the exposure variable to compute PRS of the exposure. This standardized structure enables proper alignment across all data types, supporting valid and reliable estimation of G×E interactions.
