#' Perform GWEIS, GWAS, PRS computation, and regression analysis for the reverse direction of causations in quntitatvie outcome and binary exposure variable.
#'
...
#' for quantitative outcomes and binary exposure variables in the revrse direction.
#'
...
#' @export
qbr_gweis <- function(plink_path, dis_snp, br_dis_cov, qr_dis_phen, output_dir, confounders) {
...
qbr_gweis <- function() {
  ...
  write.table(
    ...
    file.path(output_dir, "covadd_qbr.txt"),
...
compute_prs <- function() {
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "covadd_qbr.txt"), "--out", file.path(output_dir, "covadd_qbr")))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "int_qbr.txt"), "--out", file.path(output_dir, "int_qbr")))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "phenadd_br.txt"), "--out", file.path(output_dir, "phenadd_qr")))

  # Scale PRS values
  prs_cov_scaled <- scale(read.table(file.path(output_dir, "covadd_qbr.sscore"))$SCORE1_AVG)
  prs_int_scaled <- scale(read.table(file.path(output_dir, "int_qbr.sscore"))$SCORE1_AVG)
...
perform_regression <- function() {
  ...
  gcim_qbr <- glm(Outcome ~ Additive_PRS + Interaction_PRS + Covariate_Pheno +
                   Interaction_PRS:Cov_PRS + Confounders,
                 family = "binomial", data = regression_data)

  return(summary(gcim_qbr))
}
