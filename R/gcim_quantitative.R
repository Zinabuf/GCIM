...
gcim_q <- function(qp_tar_phen, qp_tar_cov, Add_PRS = NULL, Int_PRS = NULL, Cov_PRS = NULL,
  ...
  read_prs_file <- function(prs_type, provided_data = NULL) {
    if(verbose) cat("Loading", prs_type, "PRS data...\n")
    ...
    if(!is.null(provided_data)) {
      if(is.data.frame(provided_data)) {
        ...
        if(ncol(provided_data) >= 3) {
          ...
          if(nrow(prs_data) > 0) {
            if(verbose) cat("Using provided data object with", nrow(prs_data), "samples.\n")
            return(prs_data)
          }
        }
        ...
        tryCatch({
          ...
          if(verbose) cat("Error reading provided file:", e$message, "\n")
        })
      }
    }
    
    # If no provided data or it failed, try to read from temp directory
    if(verbose) cat("Searching for", prs_type, "PRS files in:", temp_dir, "\n")
    ...
    possible_files <- c(
      ...
      file.path(temp_dir, paste0(prs_type, "_int.txt")),       # Interaction effects from GWEIS
      file.path(temp_dir, paste0(prs_type, "_trd.txt")),       # additive effects from GWAS
    ...
    for(file_path in possible_files) {
      if(file.exists(file_path)) {
        ...
        tryCatch({
          ...
          if(verbose) cat("Successfully loaded", nrow(prs_data), "samples for", prs_type, "PRS from file.\n")
          return(prs_data)
