#' Perform GWEIS for quantitative outcome and binary exposure variables.
#'
...
#' @export
qbp_gweis <- function(plink_path, dis_snp, qp_dis_phen, bp_dis_cov, output_dir) {
...
#' @export
qbp_prs <- function(plink_path, tar_snp, output_dir) {
  # Create output directory if it doesn't exist
  if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)

  # Compute PRS for additive, interaction, and covariate scores
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "phenadd_qbp.txt"),
               "--out", file.path(output_dir, "add_qbp")))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "int_qbp.txt"),
               "--out", file.path(output_dir, "int_qbp")))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "covadd_bp.txt"),
               "--out", file.path(output_dir, "covadd_bp")))

  # Scale PRS values
  prs_add <- scale(read.table(file.path(output_dir, "add_qbp.sscore"))$SCORE1_AVG)
  prs_int <- scale(read.table(file.path(output_dir, "int_qbp.sscore"))$SCORE1_AVG)
  prs_cov <- scale(read.table(file.path(output_dir, "covadd_bp.sscore"))$SCORE1_AVG)

  # Save scaled PRS values
  write.table(prs_add, file.path(output_dir, "add_qbp_scaled.txt"), row.names = FALSE, col.names = FALSE)
  write.table(prs_int, file.path(output_dir, "int_qbp_scaled.txt"), row.names = FALSE, col.names = FALSE)
...
#' @export
gcim_qbp <- function(qp_tar_phen, bp_tar_cov, prs_add_scaled, prs_int_scaled, prs_cov_scaled, confounders) {
