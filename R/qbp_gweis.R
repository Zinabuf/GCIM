...
qbp_prs <- function(plink_path, tar_snp, output_dir) {
  ...
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "phenadd_qbp.txt"),
               "--out", file.path(output_dir, "add_qbp"), "&> add_qbp.log"))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "int_qbp.txt"),
               "--out", file.path(output_dir, "int_qbp"), "&> int_qbp.log"))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "covadd_bp.txt"),
               "--out", file.path(output_dir, "covadd_bp"), "&> covadd_bp.log"))

  # Check if PRS files exist before reading
  prs_files <- list(
    prs_add = file.path(output_dir, "add_qbp.sscore"),
    prs_int = file.path(output_dir, "int_qbp.sscore"),
...
#' @export
gcim_qbp <- function(qp_tar_phen, bp_tar_cov, Add_PRS, Int_PRS, Cov_PRS, confounders ) {
  # Load phenotype and covariate data
  outcome_qp_data <- read.table("qp_tar_phen", header = TRUE, stringsAsFactors = FALSE)
