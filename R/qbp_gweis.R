#' Perform GWEIS, GWAS, PRS computation, and regression analysis for the proposed direction of causations in quantitative outcome and binary exposure variable.
#'
...
#' @param qp_dis_phen File path for the phenotype data of the binary outcome in the discovery dataset.
#' @param qp_tar_phen File path for the phenotype data of the binary outcome in the target dataset.
#' @param bp_dis_cov File path for covariate data (exposure and confounders) in the discovery dataset for quatitative exposure.
#' @param bp_tar_cov File path for covariate data (exposure and confounders) in the target dataset for quatitative exposure.
...
#' @export
qbp_gweis <- function(plink_path, dis_snp, qp_dis_phen, bp_dis_cov, output_dir, confounders) {
  # Ensure output directory exists
  if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)
}
bp_gwas <- function(plink_path, dis_snp, bp_dis_cov, output_dir, confounders) {
  # Ensure output directory exists
  if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)
}
...
qbp_gweis <- function() {
  system(paste(
    ...
    "--out", file.path(output_dir, "qbp_gweis"), "&> plink.log"
  ))

  # Load GWEIS results and process
  gweis_result <- read.table(file.path(output_dir, "qbp_gweis.PHENO1.glm.linear"),
  ...
  additive_outcome <- gweis_result[gweis_result$TEST == "ADD", ]
    ...
    file.path(output_dir, "phenadd_qbp.txt"),
  ...
  interaction_outcome <- gweis_result[gweis_result$TEST == "ADDxCOVAR1", ]
    ...
    file.path(output_dir, "int_qbp.txt"),
    row.names = FALSE, sep = " ", quote = FALSE
  )
}
...
compute_prs <- function() {
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "phenadd_qbp.txt"), "--out", file.path(output_dir, "add_qbp")))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "int_qbp.txt"), "--out", file.path(output_dir, "int_qbp")))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "covadd_bp.txt"), "--out", file.path(output_dir, "covadd_bp")))

  # Scale PRS values
  prs_add_scaled <- scale(read.table(file.path(output_dir, "add_qbp.sscore"))$SCORE1_AVG)
  prs_int_scaled <- scale(read.table(file.path(output_dir, "int_qbp.sscore"))$SCORE1_AVG)
...
perform_regression <- function() {
  ...
  gcim_qbp <- lm(Outcome ~ Additive_PRS + Interaction_PRS + Covariate_Pheno +
                   Interaction_PRS:Cov_PRS + Confounders,
                  data = regression_data)

  return(summary(gcim_qbp))
}
