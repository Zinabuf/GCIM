#' Perform GWEIS, GWAS, PRS computation, and regression analysis for the proposed direction of causations in quantitative outcome and quantitative exposure variable for thre proposed diections.
#'
...
#' for quantitative outcomes and quantitative exposure variables for the proposed direction.
#'
...
#' @param qp_dis_phen File path for the phenotype data of the quantitative outcome in the discovery dataset.
#' @param qp_tar_phen File path for the phenotype data of the quntitative outcome in the target dataset.
...
#' @export
qqp_gweis <- function(plink_path, dis_snp, qp_dis_phen, qp_dis_cov, output_dir, confounders) {
...
compute_prs <- function() {
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "phenadd_qqp.txt"), "--out", file.path(output_dir, "add_qqp")))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "int_qqp.txt"), "--out", file.path(output_dir, "int_qqp")))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "covadd_qp.txt"), "--out", file.path(output_dir, "covadd_qp")))

  # Scale PRS values
  prs_add_scaled <- scale(read.table(file.path(output_dir, "add_qqp.sscore"))$SCORE1_AVG)
  prs_int_scaled <- scale(read.table(file.path(output_dir, "int_qqp.sscore"))$SCORE1_AVG)
...
perform_regression <- function() {
  ...
  gcim_qqp <- lm(Outcome ~ Additive_PRS + Interaction_PRS + Covariate_Pheno +
                   Interaction_PRS:Cov_PRS + Confounders,
                  data = regression_data)

  return(summary(gcim_qqp))
}
