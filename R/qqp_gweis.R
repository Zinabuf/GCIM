...
qqp_prs <- function(plink_path, tar_snp, output_dir) {
  ...
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "phenadd_qqp.txt"),
               "--out", file.path(output_dir, "add_qqp"), "&> add_qqp.log"))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "int_qqp.txt"),
               "--out", file.path(output_dir, "int_qqp"), "&> int_qqp.log"))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "covadd_qp.txt"),
               "--out", file.path(output_dir, "covadd_qp"), "&> covadd_qp.log"))

  # Check if PRS files exist before reading
  prs_files <- list(
    prs_add = file.path(output_dir, "add_qqp.sscore"),
    prs_int = file.path(output_dir, "int_qqp.sscore"),
...
#' @export
gcim_qqp <- function(qp_tar_phen, qp_tar_cov, Add_PRS, Int_PRS, Cov_PRS, confounders ) {
