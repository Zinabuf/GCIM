...
#' }
gcim_q <- function(qp_tar_phen, qp_tar_cov, Add_PRS = NULL, Int_PRS = NULL, Cov_PRS = NULL,
  ...
  tryCatch({
    ...
    # Normality test for residuals (Shapiro-Wilk if n < 5000, otherwise Kolmogorov-Smirnov)
    normality_test <- if(nrow(merged_data) < 5000) {
      stats::shapiro.test(residuals)
    } else {
      stats::ks.test(residuals, "pnorm", mean(residuals), stats::sd(residuals))
    }
...
#' Extract PRS data from GCIM quantitative results for further analysis
#'
...
#'
#' @param gcim_result Result object from gcim_q function
...
#' }
extract_prs_data_q <- function(gcim_result) {
...
#' Plot diagnostic plots for GCIM quantitative model
#'
#' This function creates diagnostic plots for the linear regression model
#' from GCIM quantitative analysis.
#'
#' @param gcim_result Result object from gcim_q function
#' @param which Integer vector specifying which plots to create (1-4)
#'   1: Residuals vs Fitted, 2: Normal Q-Q plot, 3: Scale-Location, 4: Cook's distance
#' @return Creates diagnostic plots
#' @export
#' @examples
#' \dontrun{
#' result <- gcim_q("Qphe_target.txt", "Qexp_target.txt", 
#'                  Add_PRS = q, Int_PRS = r, Cov_PRS = p)
#' plot_gcim_diagnostics(result)
#' }
plot_gcim_diagnostics <- function(gcim_result, which = 1:4) {
  if(!"model" %in% names(gcim_result)) {
    stop("Invalid GCIM result object. Missing model.")
  }
  
  if("error" %in% names(gcim_result)) {
    stop("Cannot create diagnostic plots for failed model fit.")
  }
  
  # Create diagnostic plots
  graphics::par(mfrow = c(2, 2))
  graphics::plot(gcim_result$model, which = which)
  graphics::par(mfrow = c(1, 1))
}

# Example usage:
# 
# # Method 1: Using PRS_quantitative function outputs directly
# q <- PRS_quantitative(plink_path, "DummyData", summary_input = add)  # Additive PRS
# r <- PRS_quantitative(plink_path, "DummyData", summary_input = gxe)  # Interaction PRS  
# p <- PRS_quantitative(plink_path, "DummyData", summary_input = trd)  # Covariate PRS
# 
