...
#' @export
gcim_q <- function(qp_tar_phen, qp_tar_cov, Add_PRS, Int_PRS, Cov_PRS, confounders) {
  temp_dir <- tempdir()  
  # Load phenotype and covariate data
  outcome_bp_data <- read.table(qp_tar_phen, header = FALSE, stringsAsFactors = FALSE)
  ...
  covariate_bp_data <- read.table(qp_tar_cov, header = FALSE, stringsAsFactors = FALSE, fill = TRUE)
