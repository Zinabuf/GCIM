...
bqp_prs <- function(plink_path, tar_snp, output_dir) {
  ...
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "phenadd_bqp.txt"),
               "--out", file.path(output_dir, "add_bqp")))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "int_bqp.txt"),
               "--out", file.path(output_dir, "int_bqp")))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "covadd_qp.txt"),
               "--out", file.path(output_dir, "covadd_qp")))

  # Scale PRS values
  prs_add <- scale(read.table(file.path(output_dir, "add_bqp.sscore"))$SCORE1_AVG)
  prs_int <- scale(read.table(file.path(output_dir, "int_bqp.sscore"))$SCORE1_AVG)
  prs_cov <- scale(read.table(file.path(output_dir, "covadd_qp.sscore"))$SCORE1_AVG)

  # Save scaled PRS values
  write.table(prs_add, file.path(output_dir, "add_bqp_scaled.txt"), row.names = FALSE, col.names = FALSE)
  write.table(prs_int, file.path(output_dir, "int_bqp_scaled.txt"), row.names = FALSE, col.names = FALSE)
...
#' @export
gcim_bqp <- function(bp_tar_phen, qp_tar_cov, prs_add_scaled, prs_int_scaled, prs_cov_scaled, confounders) {
