...
#' @export
bqp_gweis <- function(plink_path, dis_snp, bp_dis_phen, qp_dis_cov, output_dir) {
...
#' @export
bqp_prs <- function(plink_path, tar_snp, output_dir) {
  # Create output directory if it doesn't exist
  if (!dir.exists(output_dir)) dir.create(output_dir, recursive = TRUE)

  # Compute PRS for additive, interaction, and covariate scores
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "phenadd_bqp.txt"),
               "--out", file.path(output_dir, "add_bqp"), "&> add_bqp.log"))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "int_bqp.txt"),
               "--out", file.path(output_dir, "int_bqp"), "&> int_bqp.log"))
  system(paste(plink_path, "--bfile", tar_snp, "--score", file.path(output_dir, "covadd_qp.txt"),
               "--out", file.path(output_dir, "covadd_qp"), "&> covadd_bqp.log"))

  # Check if PRS files exist before reading
  prs_files <- list(
    prs_add = file.path(output_dir, "add_bqp.sscore"),
    prs_int = file.path(output_dir, "int_bqp.sscore"),
    prs_cov = file.path(output_dir, "covadd_qp.sscore")
  )
  prs_values <- list()
...
#' @export
gcim_bqp <- function(bp_tar_phen, qp_tar_cov, Add_PRS, Int_PRS, Cov_PRS, confounders ) {
  ...
  covariate_qp_data <- read.table("qp_tar_cov", header = TRUE, stringsAsFactors = FALSE, fill = TRUE)
  ...
  regression_data <- data.frame(
    ...
    Int_PRS = prs_int[, 3],
    Cov_PRS = prs_cov[, 3],
    Covariate_Pheno = covariate_qp_data[, 3] # Adjust column index as needed
  )

  # Add confounders dynamically based on the input `conf_`
  confounders <- covariate_qp_data[, 4:19]
